...
class DrawingClassifier:
    ...
    def predict(self):
        if self.clf is not None:
            ...
            if prediction[0] == 1:
                tkinter.messagebox.showinfo("Drawing Classifier", f"The drawing is probably a {self.class1}",
                                            parent=self.root)
            elif prediction[0] == 2:
                tkinter.messagebox.showinfo("Drawing Classifier", f"The drawing is probably a {self.class2}",
                                            parent=self.root)
            elif prediction[0] == 3:
                tkinter.messagebox.showinfo("Drawing Classifier", f"The drawing is probably a {self.class3}",
                                            parent=self.root)
        else:
            tkinter.messagebox.showwarning("Drawing Classifier", "Train the model first!", parent=self.root)

    def load_model(self):
        file_path = filedialog.askopenfilename()
        with open(file_path, "rb") as f:
            self.clf = pickle.load(f)
        tkinter.messagebox.showinfo("Drawings classifier", "Model successfully loaded!", parent=self.root)
    ...
    def save_everything(self):
        ...
        with open(f"{self.proj_name}/{self.proj_name}_data.pickle", "wb") as f:
            pickle.dump(data, f)
        tkinter.messagebox.showinfo("Drawings classifier", "Project successfully saved!", parent=self.root)
